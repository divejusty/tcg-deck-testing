...
const saveForm = () => {
    form.delete(route('archetypes.destroy', {archetype: props.archetype.id}), {
        preserveScroll: true,
        onSuccess: () => {
            form.reset()
            closeModal()
    ...
    <Modal :show="modalVisible" @close="closeModal">
        <div class="p-6">
            <h2 class="text-lg font-medium text-gray-900 dark:text-gray-100">
                Delete archetype "{{ `${archetype.name}` }}"
            </h2>

            <p class="mt-1 text-sm text-gray-600 dark:text-gray-400">
                Are you sure you want to delete this archetype? This action cannot be reversed.
            </p>
